...
const Header = () => {
  ...
  // Estado del usuario (Para mostrar avatar/nombre en el Header)
    ...
    toast(
      (t) => (
        <div className="flex flex-col gap-2 font-kodchasan">
          <span className="font-semibold text-gray-800">¿Cerrar sesión?</span>
  ...
  const performLogout = () => {
    ...
    // 2. DISPARAMOS EL EVENTO PARA BLOQUEAR LA TABLA AL INSTANTE
  ...
  // --- 2. LÓGICA DE LOGIN ---
  // Esta función se ejecuta cuando el LoginForm termina con éxito
  const handleLoginSuccess = (userData) => {
    setUser(userData);

    // 3. DISPARAMOS EL EVENTO PARA DESBLOQUEAR LA TABLA AL INSTANTE
    window.dispatchEvent(new Event("auth-change"));
  };
